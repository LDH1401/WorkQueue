  ...
  return (
    <>
      <header className="page-head">
        <div>
          <h1>Bảng Kanban</h1>
          <p className="muted">Kéo thả thẻ để đổi trạng thái công việc.</p>
        </div>
        ...
        <div className="board">
          {STATUSES.map((status) => {
            ...
            return (
              <section
                key={status.value}
                className={`column${drag ? ' column--dropping' : ''}`}
                onDragOver={(e) => {
                  e.preventDefault();
              ...
              >
                ...
                <div className="column__body">
                  {items.map((task, i) => (
                    ...
                    >
                      {over?.status === status.value && over.index === i && <div className="drop-line" />}
                      <TaskCard
                        task={task}
                        draggable
                  ...
                  {over?.status === status.value && over.index >= items.length && <div className="drop-line" />}

                  {items.length === 0 && <p className="column__empty">Kéo thẻ vào đây</p>}
                </div>

                <button type="button" className="column__add" onClick={() => setDialog({ status: status.value })}>
                  + Thêm
                </button>
              </section>
            );
